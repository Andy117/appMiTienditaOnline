...
export const updateOrderWithDetails = async(req, res) => {
    const { id } = req.params
    try {
        const[order] = await sequelize.query(
            'SELECT * FROM Orden WHERE idOrden = :id',
        ...
        await sequelize.query(
            'EXEC sp_ActualizarOrdenConDetalles @idOrden=:id, @nombreCompleto=:nombreCompleto, @direccionOrden=:direccionOrden, @telefonoOrden=:telefonoOrden, @correoElectronicoOrden=:correoElectronicoOrden, @fechaEntregaOrden=:fechaEntregaOrden, @totalOrden=:totalOrden, @DetallesJSON=:DetallesJSON',
            {
                replacements:{
                    nombreCompleto: validatedData.nombreCompleto,
                    direccionOrden: validatedData.direccionOrden,
        ...
        res.status(500).json({ message: 'Hubo un problema al actualizar los datos de la orden: ', error: error.message})
    }
}
...
export const getAllOrdersForClient = async (req, res) => {
    const { idUsuario } = req.user

    try {
        const results = await sequelize.query('EXEC sp_ObtenerOrdenesPorCliente @idUsuarios=:idUsuario', {
...
export const cancelOrder = async (req, res) => {
    ...
    try {
        await sequelize.query(
            'EXEC sp_CancelarOrden @idOrden=:id',
            {
                replacements: { id }
            }
        );
